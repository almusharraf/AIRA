...
const ManageEvents: React.FC = () => {
  ...
  const handleDeleteAppointment = async (id: string) => {
    try {
      await axiosInstance.delete(`delete_Event/${id}/`);
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axiosInstance.get('AddEvent/');
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Colors.background }]}>
      <ScrollView style={{ flex: 1 }}>
        <Text style={[styles.headerTitle, { color: Colors.authTitleColor }]}>{"Manage Events"}</Text>
        ...
        <NewEvent isVisible={isModalVisible} onClose={handleCloseModal} onAdd={handleAddAppointment} />
